...
export default function JobList() {
  ...
  const [deptOption, setDeptOption] = useState("all");
  const [searchTerm, setSearchTerm] = useState("");
  ...
  const selectHandler = (event) => {
    ...
    if (selectedDept === "all") {
      ...
      setFilteredJobs(
        data.filter(
          (job) => job.department?.name?.toLowerCase() === selectedDept
        )
      );
    }
  };
      ...
      {loading ? (
        ...
        <div className="flex flex-col space-y-4 w-full">
          <div className="flex items-center bg-white border border-gray-800 mt-4">
            ...
            {/* Search Input */}
            <div className="w-2/3 relative">
              <div className="absolute flex items-center top-0 bottom-0 px-2">
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  fill="none"
                  viewBox="0 0 24 24"
              ...
              <input
                value={searchTerm}
                onChange={inputHandler}
                className="w-full px-8 py-2 border border-gray-300 rounded"
                type="text"
                placeholder="Search Job Here"
              />
            </div>
          ...
          <div className="relative w-full overflow-x-auto">
            <table className="w-full table-auto border-collapse border border-gray-200">
              <thead className="text-xl border-b border-gray-200">
                <tr>
                  <th className="py-2 text-start px-3">Title</th>
                  <th className="py-2 text-start px-3">Department</th>
                  <th className="py-2 text-start px-3">Start Date</th>
                  <th className="py-2 text-start px-3">End Date</th>
                </tr>
              </thead>
              <tbody className="text-center text-lg">
                {filteredJobs.map((item, index) => (
